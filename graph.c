...
void traceKeyHandler(int key, khdata *data)
{
	...
	switch (key) {
		...
		case 'h': *data->trace -= 4.0*step; break;
		case 'l': *data->trace += 4.0*step; break;
	}
...
void defaultKeyHandler(int key, khdata *data)
{
	...
	switch (key) {
		...
		case 'k':	yshift = 1; break;
		case 'j':	yshift = -1; break;
		case 'h':	xshift = -1; break;
		case 'l':	xshift = 1; break;
	}
...
int main(int argc, char *argv[])
{
	...
	printw("Created by flarn2006 <flarn2006@gmail.com>\n");
	printw("Compiled with libmatheval (http://www.gnu.org/software/libmatheval/)\n");
	printw("Also requires ncurses (Google it.)\n");
	getch();
